workspace "Сайт конференции" {
    model {
        ...
        searchUserMask -> dbUser "Читает данные о пользователях по имени"
        
        guest -> searchUserLogin "Поиск пользователя по логину"
        searchUserLogin -> dbUser "Читает данные о пользователях по логину"
        
        guest -> listDoclad "Поиск доступных всех докладов"
        listDoclad -> dbDoclad "Читает данные о доступных всех докладах"
        
        guest -> listDocladConference "Поиск доступных докладов в конференции"
        listDocladConference -> dbDoclad "Читает данные о доступных докладах в конференции"
    }
